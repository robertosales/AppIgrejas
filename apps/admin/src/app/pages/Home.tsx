...
import { Bell, Heart, MessageCircle, CalendarDays, Instagram, Phone, ChevronRight } from "lucide-react";
  ...
  return (
    <div className="flex flex-col min-h-full">
      ...
      <header className="px-6 pt-12 pb-4 bg-slate-900 text-white flex justify-between items-center rounded-b-[32px] shadow-sm relative z-10">
        <div>
          <p className="text-amber-400 text-xs font-medium tracking-widest uppercase mb-1">Bem-vindo</p>
          <h1 className="text-xl font-serif">Comunidade Vida</h1>
        </div>
        <button 
      ...
      <div className="px-5 py-6 space-y-8">
        ...
        <div>
          ...
          <div className="space-y-3">
            {[
              { title: "Culto de Jovens", date: "Sáb, 20h", loc: "Templo Principal" },
              { title: "Escola Bíblica", date: "Dom, 9h", loc: "Salas Anexas" }
            ].map((event, i) => (
              <div key={i} onClick={() => navigate("/app/agenda/1")} className="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm flex items-center gap-4 cursor-pointer active:scale-[0.98] transition-transform">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-slate-500 text-sm mt-0.5">{event.date} • {event.loc}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="bg-slate-900 rounded-3xl p-6 text-center text-white relative overflow-hidden">
          ...
          <h3 className="relative z-10 font-serif text-lg mb-4">Siga nossas redes</h3>
          <div className="relative z-10 flex justify-center gap-4">
            <button className="flex items-center gap-2 bg-slate-800 hover:bg-slate-700 px-4 py-2.5 rounded-full transition-colors text-sm font-medium">
              <Instagram size={18} className="text-amber-500" /> Instagram
            </button>
            <button className="flex items-center gap-2 bg-slate-800 hover:bg-slate-700 px-4 py-2.5 rounded-full transition-colors text-sm font-medium">
              <Phone size={18} className="text-amber-500" /> WhatsApp
            </button>
          </div>
        </div>
